...
type GdprPageSearchParams = RouteSearchParams;

type GdprPageProps = {
  searchParams?: GdprPageSearchParams;
};

export default function GdprPage({ searchParams }: GdprPageProps) {
  ...
  return (
    <div className="bg-white text-slate-900">
      <SiteHeader locale={locale} pathname="/gdpr" searchParams={currentSearchParams} />
      <main className="flex flex-col">
        <section className="page-section border-b border-slate-200 bg-slate-50">
          <div className="mx-auto max-w-5xl px-6 py-16 sm:py-20">
            ...
            <div className="space-y-5">
              <h1 className="text-4xl font-semibold sm:text-5xl">{t("privacyPage.hero.title")}</h1>
              <p className="text-lg text-slate-600">{t("privacyPage.hero.description")}</p>
              <p className="text-sm text-slate-500">{t("privacyPage.meta.effectiveDate")}</p>
            </div>
          </div>
        </section>
        ...
        <section className="page-section bg-white">
          <div className="mx-auto max-w-5xl space-y-10 px-6 py-16 sm:py-20">
            {sectionKeys.map((key) => {
              const title = t(`privacyPage.sections.${key}.title` as const);
              const body = t(`privacyPage.sections.${key}.body` as const);
              const list = t(`privacyPage.sections.${key}.list` as const);
              const listTitle = t(`privacyPage.sections.${key}.listTitle` as const);

              const paragraphs = splitIntoParagraphs(body);
              const listItems = splitIntoList(list);
              const showList = listItems.length > 0 && list !== `privacyPage.sections.${key}.list`;
              const listHeading = listTitle !== `privacyPage.sections.${key}.listTitle` ? listTitle : undefined;
        ...
        <section className="page-section border-y border-slate-200 bg-slate-50">
          <div className="mx-auto max-w-5xl px-6 py-16 sm:py-20">
            <Card className="bg-white">
              <CardHeader>
                <CardTitle className="text-2xl font-semibold sm:text-3xl">{t("privacyPage.cta.title")}</CardTitle>
                <p className="text-base text-slate-600">{t("privacyPage.cta.description")}</p>
              </CardHeader>
              <CardContent className="grid gap-6 md:grid-cols-3">
                <div className="rounded-lg border border-slate-200 p-4">
                  <div className="text-xs font-semibold uppercase tracking-wide text-slate-500">
                    {t("privacyPage.cta.phoneLabel")}
                  </div>
                ...
                <div className="rounded-lg border border-slate-200 p-4">
                  <div className="text-xs font-semibold uppercase tracking-wide text-slate-500">
                    {t("privacyPage.cta.addressLabel")}
                  </div>
                  <div className="mt-2 flex items-start gap-2 text-sm text-slate-700">
                    <MapPin className="mt-0.5 h-4 w-4 text-emerald-600" />
                    <span>{t("privacyPage.cta.addressValue")}</span>
                  </div>
                  <p className="mt-3 text-xs text-slate-500">{t("privacyPage.cta.note")}</p>
                </div>
              </CardContent>
            </Card>
          </div>
...
function splitIntoParagraphs(value: string) {
  if (!value || value.startsWith("privacyPage")) {
    return [];
  }
